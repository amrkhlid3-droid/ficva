...
describe('Commands', () => {
  ...
  describe('AddObjectCommand', () => {
    it('should add object on execute', () => {
      const command = new AddObjectCommand(mockCanvas as Canvas, mockObject as FabricObject);
      command.execute();
  ...
  describe('ModifyObjectCommand', () => {
    it('should apply new properties on execute', () => {
      ...
      const command = new ModifyObjectCommand(mockObject as FabricObject, newProps, originalProps);
      ...
      expect(mockCanvas.requestRenderAll).toHaveBeenCalled();
      expect(mockCanvas.fire).toHaveBeenCalledWith('object:modified', { target: mockObject });
    });
