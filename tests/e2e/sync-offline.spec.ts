...
test.describe("Sync & Offline Capabilities", () => {
    ...
    await page.route(`**/api/projects/${projectId}`, async route => {
        if (route.request().method() === "GET") {
                ...
                body: JSON.stringify({
                    ...
                    data: {
                        ...
                        json: {
                            pages: [{
                                id: "page-1",
                                json: { version: "5.3.0", objects: [], backgroundColor: "#ffffff" }
                            }],
                            activePageId: "page-1"
                        },
            ...
            await route.fulfill({
                status: 200,
                contentType: "application/json",
                body: JSON.stringify({ success: true, data: { updatedAt: new Date().toISOString() } })
            })
        } else {
  ...
  test("Offline Persistence: Changes should survive reload when offline", async ({ page }) => {
  ...
  test("Immediate Sync: Adding a page should trigger immediate request", async ({ page }) => {
      await expect(page.locator("canvas")).toBeVisible()

      // Monitor network requests
      const requestPromise = page.waitForRequest(request =>
          (request.url().includes("/properties") || request.url().includes("/projects") || request.url().includes("/api/projects"))
  ...
  test("Conflict Resolution: Local newer data should override server", async ({ page }) => {
      const conflictProjectId = "conflict-test-" + Date.now()

      // 1. Inject Local Storage Data (Newer)
      const localData = {
          pages: [{ id: "p1", json: {} }, { id: "p2", json: {} }], // 2 pages locally
          activePageId: "p1",
      ...
      await page.route(`**/api/projects/${conflictProjectId}`, async route => {
         if (route.request().method() === "GET") {
             ...
             await route.fulfill({ status: 200, body: JSON.stringify({ success: true, data: { updatedAt: new Date().toISOString() } }) })
         } else {
             await route.continue()
         }
      })
