...
test.describe('Canvas Background Persistence', () => {
  ...
  test('should persist background color when adding objects', async ({ page }) => {
    ...
    await expect(async () => {
      ...
      return bgColor === '#ff0000' || bgColor === 'rgba(255, 0, 0, 1)' || bgColor === 'red';
    }).toPass();
    ...
    expect(bgColorAfterAdd === '#ff0000' || bgColorAfterAdd === 'rgba(255, 0, 0, 1)' || bgColorAfterAdd === 'red').toBeTruthy();
